...
class TransformerModel(nn.Module):
    def __init__(self, time_step, input_dims, hidden_size, embed_dim, output_dim, out_dropout):
        """
        ...
        # RNN networks
        self.lstm = nn.LSTM(self.embed_dim, 128, bidirectional=True)

        # Projection layers
        self.proj = nn.Linear(self.d_x, self.embed_dim)
        
        self.out_fc1 = nn.Linear(128*2, h_out)
    ...
    def forward(self, x):
        ...
        output = self.out_fc2(self.out_dropout(F.relu(self.out_fc1(output))))
